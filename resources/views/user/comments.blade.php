...
@section('content')
                ...
                <table class="table table-striped">
                    ...
                    @foreach($comments as $comment)
                        <tr>
                            ...
                            <td>
                                {{--<form id="deleteComment-{{ $comment->id }}" action="{{ route('userCommentDelete', $comment->id) }}" method="POST">--}}
                                    {{--@csrf</form>--}}
                                {{--<button type="button" onclick="document.getElementById('deleteComment-{{ $comment->id }}').submit()" class="btn btn-danger">X</button>--}}
                                <button type="button" class="btn btn-danger" data-toggle="modal" data-target="#deleteCommentUser-{{$comment->id}}">
                                    X
                                </button>
                            </td>
                        </tr>
    ...
    @foreach($comments as $comment)
        <!-- Modal -->
        <div class="modal fade" id="deleteCommentUser-{{$comment->id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
